...
class CI_Input
{
    ...
    public function __construct ()
    {
        ...
        //$this->_sanitize_globals();  //  by tangjian 20120328
    }
    ...
    function ip_address ()
    {
        ...
        if (config_item('proxy_ips') != '' &&
                 $this->server('HTTP_X_FORWARDED_FOR') &&
                 $this->server('REMOTE_ADDR')) {
            $proxies = preg_split('/[\s,]/', config_item('proxy_ips'), - 1, 
                    PREG_SPLIT_NO_EMPTY);
        ...
        } elseif ($this->server('REMOTE_ADDR') and
    ...
    protected function _valid_ipv4 ($ip)
    {
        ...
        foreach ($ip_segments as $segment) {
            ...
            if ($segment == '' or preg_match("/[^0-9]/", $segment) or
                     $segment > 255 or strlen($segment) > 3) {
                return FALSE;
            }
        }
    ...
    function _sanitize_globals ()
    {
        ...
        if ($this->_allow_get_array == FALSE) {
            ...
            if (is_array($_GET) and count($_GET) > 0) {
                foreach ($_GET as $key => $val) {
                    $_GET[$this->_clean_input_keys($key)] = $this->_clean_input_data(
                            $val);
                }
            }
        }
        ...
        if (is_array($_POST) and count($_POST) > 0) {
            foreach ($_POST as $key => $val) {
                $_POST[$this->_clean_input_keys($key)] = $this->_clean_input_data(
                        $val);
            }
        }
        ...
        if (is_array($_COOKIE) and count($_COOKIE) > 0) {
            ...
            foreach ($_COOKIE as $key => $val) {
                $_COOKIE[$this->_clean_input_keys($key)] = $this->_clean_input_data(
                        $val);
            }
        }
    ...
    function _clean_input_data ($str)
    {
        if (is_array($str)) {
            ...
            foreach ($str as $key => $val) {
                $new_array[$this->_clean_input_keys($key)] = $this->_clean_input_data(
                        $val);
            }
            return $new_array;
        }
        ...
        if (function_exists('apache_request_headers')) {
            ...
            foreach ($_SERVER as $key => $val) {
                if (strncmp($key, 'HTTP_', 5) === 0) {
                    $headers[substr($key, 5)] = $this->_fetch_from_array(
