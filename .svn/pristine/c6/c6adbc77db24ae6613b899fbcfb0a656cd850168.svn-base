...
class CI_DB_cubrid_forge extends CI_DB_forge
{
                        ...
                        switch ($attributes['TYPE']) {
                            ...
                            case 'numeric':
                                $sql .= '(' .
                                         ...
                                         ')';
                                break;
                            case 'enum': // As of version 8.4.0 CUBRID does not
                                         // support
                                         // enum data type.
                                break;
                            case 'set':
            ...
            $if_not_exists)
    {
        ...
        if (is_array($keys) && count($keys) > 0) {
            foreach ($keys as $key) {
                if (is_array($key)) {
                    $key_name = $this->db->_protect_identifiers(
                            implode('_', $key));
    ...
    {
        $sql = 'RENAME TABLE ' . $this->db->_protect_identifiers($table_name) .
                 " AS " . $this->db->_protect_identifiers($new_table_name);
        return $sql;
    }
}
