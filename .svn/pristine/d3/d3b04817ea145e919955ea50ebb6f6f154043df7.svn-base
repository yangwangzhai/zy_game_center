...
class CI_Xmlrpc
{
    ...
    function values_parsing ($value, $return = FALSE)
    {
        if (is_array($value) && array_key_exists(0, $value)) {
            if (! isset($value['1']) or
                     (! isset($this->xmlrpcTypes[$value['1']]))) {
                if (is_array($value[0])) {
                    ...
                    $temp = new XML_RPC_Values($value['0'], 'string');
                     ...
                     ($value['1'] == 'struct' or $value['1'] == 'array')) {
...
class XML_RPC_Client extends CI_Xmlrpc
{
    ...
    function send ($msg)
    {
        if (is_array($msg)) {
            // Multi-call disabled
            $r = new XML_RPC_Response(0, $this->xmlrpcerr['multicall_recursion'], 
    ...
    function sendPayload ($msg)
    {
        ...
        if (! is_resource($fp)) {
            ...
            $r = new XML_RPC_Response(0, $this->xmlrpcerr['http_error'], 
...
class XML_RPC_Response
{
        ...
        if ($array !== FALSE && is_array($array)) {
            while (list ($key) = each($array)) {
                if (is_array($array[$key])) {
                    ...
                    $array[$key] = ($this->xss_clean) ? $CI->security->xss_clean(
                            $array[$key]) : $array[$key];
                }
            }
    ...
    function iso8601_decode ($time, $utc = 0)
    {
        ...
        if (preg_match(
            ...
            $t = $fnc($regs[4], $regs[5], $regs[6], $regs[2], $regs[3], 
                    $regs[1]);
        }
        return $t;
    }
...
class XML_RPC_Message extends CI_Xmlrpc
{
    ...
    function createPayload ()
    {
        ...
        $this->payload .= '<methodName>' . $this->method_name .
                 "</methodName>\r\n";
        $this->payload .= "<params>\r\n";
    ...
    function parseResponse ($fp)
    {
        ...
        if ($data == "") {
            ...
            $r = new XML_RPC_Response(0, $this->xmlrpcerr['no_data'], 
                    $this->xmlrpcstr['no_data']);
            return $r;
        }
        ...
        if (! xml_parse($parser, $data, count($data))) {
            ...
            // error_log($errstr);
            $r = new XML_RPC_Response(0, $this->xmlrpcerr['invalid_return'], 
                    $this->xmlrpcstr['invalid_return']);
    ...
    function open_tag ($the_parser, $name, $attrs)
    {
        ...
        if (count($this->xh[$the_parser]['stack']) == 0) {
            ...
            if (! in_array($this->xh[$the_parser]['stack'][0], 
                    $this->valid_parents[$name], TRUE)) {
                $this->xh[$the_parser]['isf'] = 2;
                $this->xh[$the_parser]['isf_reason'] = "XML-RPC element $name cannot be child of " .
                         $this->xh[$the_parser]['stack'][0];
                return;
            }
        }
        ...
        switch ($name) {
            ...
            case 'BASE64':
                ...
                if ($name == 'STRING') {
                    ...
                    $this->xh[$the_parser]['value'] = base64_decode(
                            $this->xh[$the_parser]['ac']);
                } elseif ($name == 'BOOLEAN') {
                ...
                $this->xh[$the_parser]['ac'] = '';
                $this->xh[$the_parser]['lv'] = 3; // indicate we've found a
                                                  // value
                break;
            case 'VALUE':
                ...
                // substitute it
                $temp = new XML_RPC_Values($this->xh[$the_parser]['value'], 
                        $this->xh[$the_parser]['vt']);
                
                if (count($this->xh[$the_parser]['valuestack']) &&
                         $this->xh[$the_parser]['valuestack'][0]['type'] ==
                         'ARRAY') {
                    // Array
            ...
            case 'PARAM':
                ...
                break;
            case 'METHODNAME':
                $this->xh[$the_parser]['method'] = ltrim(
                        $this->xh[$the_parser]['ac']);
                break;
            case 'PARAMS':
            case 'FAULT':
        ...
        if ($array !== FALSE && is_array($array)) {
            while (list ($key) = each($array)) {
                if (is_array($array[$key])) {
                    ...
                    $array[$key] = ($key == 'bits' or $this->xss_clean == FALSE) ? $array[$key] : $CI->security->xss_clean(
                            $array[$key]);
                }
            }
            ...
            for ($i = 0; $i < count($this->params); $i ++) {
                ...
                if (is_array($a_param)) {
                    ...
                    $parameters[] = ($this->xss_clean) ? $CI->security->xss_clean(
                            $a_param) : $a_param;
                }
            }
        }
...
class XML_RPC_Values extends CI_Xmlrpc
{
    ...
    function addScalar ($val, $type = 'string')
    {
        ...
        if ($type == $this->xmlrpcBoolean) {
            if (strcasecmp($val, 'true') == 0 or $val == 1 or
    ...
    {
        if ($this->mytype != 0) {
            echo '<strong>XML_RPC_Values</strong>: already initialized as a [' .
                     $this->kindOf() . ']<br />';
            return 0;
        }
