...
class CI_Image_lib
{
    ...
    var $master_dim = 'auto'; // auto, height, or width. Determines what to
                                // use as the master dimension
    var $rotation_angle = '';
    ...
    function initialize ($props = array())
    {
        ...
        if ($this->new_image == '') {
            ...
            if (strpos($this->new_image, '/') === FALSE and
                     strpos($this->new_image, '\\') === FALSE) {
                $this->dest_folder = $this->source_folder;
                $this->dest_image = $this->new_image;
            } else {
                if (function_exists('realpath') and
                         @realpath($this->new_image) !== FALSE) {
                    $full_dest_path = str_replace("\\", "/", 
                            realpath($this->new_image));
                } else {
                ...
                if (! preg_match("#\.(jpg|jpeg|gif|png)$#i", $full_dest_path)) {
                    ...
                    $this->dest_folder = str_replace($this->dest_image, '', 
                            $full_dest_path);
                }
            }
        }
        ...
        if ($this->quality == '' or $this->quality == 0 or
    ...
    function rotate ()
    {
        ...
        if ($this->rotation_angle == '' or
                 ! in_array($this->rotation_angle, $degs)) {
        ...
        if ($this->image_library == 'imagemagick' or
    ...
    function image_process_gd ($action = 'resize')
    {
        ...
        if ($this->dynamic_output === FALSE) {
            if ($this->orig_width == $this->width and
    ...
    function image_process_netpbm ($action = 'resize')
    {
        ...
        $cmd = $this->library_path . $cmd_in . ' ' . $this->full_src_path . ' | ' .
                 $cmd_inner . ' | ' . $cmd_out . ' > ' . $this->dest_folder .
                 'netpbm.tmp';
    ...
    function overlay_watermark ()
    {
        ...
        if ($alpha > 0) {
            ...
            imagecopy($src_img, $wm_img, $x_axis, $y_axis, 0, 0, $wm_width, 
                    $wm_height);
        } else {
            ...
            imagecolortransparent($wm_img, 
                    imagecolorat($wm_img, $this->wm_x_transp, 
                            $this->wm_y_transp));
    ...
    function text_watermark ()
    {
        ...
        if ($this->wm_use_truetype == TRUE and ! file_exists(
                $this->wm_font_path)) {
        ...
        switch ($this->wm_hor_alignment) {
            ...
            case "R":
                ...
                $x_axis += ($this->orig_width - $fontwidth *
                         strlen($this->wm_text));
                break;
            case "C":
                if ($this->wm_use_drop_shadow)
    ...
    function image_save_gd ($resource)
    {
        switch ($this->image_type) {
            ...
            case 2:
                ...
                if (! @imagejpeg($resource, $this->full_dst_path, 
                        $this->quality)) {
                    $this->set_error('imglib_save_failed');
                    return FALSE;
                }
    ...
    {
        if (! is_numeric($this->width) or ! is_numeric($this->height) or
                 $this->width == 0 or $this->height == 0)
            return;
        
        if (! is_numeric($this->orig_width) or ! is_numeric($this->orig_height) or
                 $this->orig_width == 0 or $this->orig_height == 0)
            return;
    ...
    function explode_name ($source_image)
    {
        ...
        $name = ($ext === FALSE) ? $source_image : substr($source_image, 0, 
                - strlen($ext));
    ...
    function set_error ($msg)
    {
        ...
        if (is_array($msg)) {
            foreach ($msg as $val) {
                ...
                $msg = ($CI->lang->line($val) == FALSE) ? $val : $CI->lang->line(
                        $val);
                $this->error_msg[] = $msg;
                log_message('error', $msg);
            }
        } else {
            $msg = ($CI->lang->line($msg) == FALSE) ? $msg : $CI->lang->line(
                    $msg);
            $this->error_msg[] = $msg;
            log_message('error', $msg);
        }
