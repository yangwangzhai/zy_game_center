...
class CI_Zip
{
    ...
    function add_dir ($directory)
    {
        foreach ((array) $directory as $dir) {
            ...
            $this->_add_dir($dir, $dir_time['file_mtime'], 
                    $dir_time['file_mdate']);
        }
    }
    ...
    function _get_mod_time ($dir)
    {
        ...
        $time['file_mtime'] = ($date['hours'] << 11) + ($date['minutes'] << 5) +
                 $date['seconds'] / 2;
        $time['file_mdate'] = (($date['year'] - 1980) << 9) + ($date['mon'] << 5) +
                 $date['mday'];
    ...
    function _add_dir ($dir, $file_mtime, $file_mdate)
    {
        ...
        $this->zipdata .= "\x50\x4b\x03\x04\x0a\x00\x00\x00\x00\x00" .
                 pack('v', $file_mtime) . pack('v', $file_mdate) . pack('V', 0) . // crc32
    ...
    function add_data ($filepath, $data = NULL)
    {
        if (is_array($filepath)) {
            foreach ($filepath as $path => $data) {
                ...
                $this->_add_data($path, $data, $file_data['file_mtime'], 
                        $file_data['file_mdate']);
            }
        } else {
            $file_data = $this->_get_mod_time($filepath);
            
            $this->_add_data($filepath, $data, $file_data['file_mtime'], 
                    $file_data['file_mdate']);
        }
    }
    ...
    function _add_data ($filepath, $data, $file_mtime, $file_mdate)
    {
        ...
        $this->zipdata .= "\x50\x4b\x03\x04\x14\x00\x00\x00\x08\x00" .
                 pack('v', $file_mtime) . pack('v', $file_mdate) .
                 pack('V', $crc32) . pack('V', $compressed_size) .
                 pack('V', $uncompressed_size) . pack('v', strlen($filepath)) . // length of filename
                pack('v', 0) .                 // extra field length
    ...
    function read_dir ($path, $preserve_filepath = TRUE, $root_path = NULL)
    {
        ...
        while (FALSE !== ($file = readdir($fp))) {
            ...
            if (@is_dir($path . $file)) {
                $this->read_dir($path . $file . "/", $preserve_filepath, 
                        $root_path);
            } else {
