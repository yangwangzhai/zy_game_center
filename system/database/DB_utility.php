...
class CI_DB_utility extends CI_DB_forge
{
    ...
    function csv_from_result ($query, $delim = ",", $newline = "\n", $enclosure = '"')
    {
        ...
        foreach ($query->list_fields() as $name) {
            $out .= $enclosure .
                     str_replace($enclosure, $enclosure . $enclosure, $name) .
                     $enclosure . $delim;
        }
        ...
        foreach ($query->result_array() as $row) {
            foreach ($row as $item) {
                $out .= $enclosure .
                         str_replace($enclosure, $enclosure . $enclosure, $item) .
                         $enclosure . $delim;
            }
            $out = rtrim($out);
            $out .= $newline;
    ...
    function xml_from_result ($query, $params = array())
    {
        ...
        foreach ($query->result_array() as $row) {
            ...
            foreach ($row as $key => $val) {
                $xml .= $tab . $tab . "<{$key}>" . xml_convert($val) .
                         "</{$key}>" . $newline;
            }
            $xml .= $tab . "</{$element}>" . $newline;
        }
    ...
    function backup ($params = array())
    {
        ...
        if (($prefs['format'] == 'gzip' and ! @function_exists('gzencode')) or
                 ($prefs['format'] == 'zip' and ! @function_exists('gzcompress'))) {
            if ($this->db->db_debug) {
