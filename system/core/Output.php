...
class CI_Output
{
    ...
    function __construct ()
    {
        ...
        if (defined('ENVIRONMENT') and
                 file_exists(APPPATH . 'config/' . ENVIRONMENT . '/mimes.php')) {
    ...
    function _display ($output = '')
    {
        ...
        if ($this->cache_expiration > 0 && isset($CI) &&
                 ! method_exists($CI, '_output')) {
        ...
        $elapsed = $BM->elapsed_time('total_execution_time_start', 
                'total_execution_time_end');
        
        if ($this->parse_exec_vars === TRUE) {
            $memory = (! function_exists('memory_get_usage')) ? '0' : round(
    ...
    function _write_cache ($output)
    {
        ...
        $uri = $CI->config->item('base_url') . $CI->config->item('index_page') .
                 $CI->uri->uri_string();
        ...
        if (flock($fp, LOCK_EX)) {
            ...
            log_message('error', 
                    "Unable to secure a file lock for file at: " . $cache_path);
            return;
        }
        fclose($fp);
    ...
    {
        $cache_path = ($CFG->item('cache_path') == '') ? APPPATH . 'cache/' : $CFG->item(
                'cache_path');
        
        // Build the file path. The file name is an MD5 hash of the full URI
        $uri = $CFG->item('base_url') . $CFG->item('index_page') .
                 $URI->uri_string;
