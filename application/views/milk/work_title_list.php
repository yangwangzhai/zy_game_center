...
function over(){
	if(confirm('您确定要结束本次活动？')){
		location.href='<?=$this->baseurl?>&m=over<?=$game_sign?>'
	}
}
function getstate(){		
...
<div class="mainbox">
 <input type="button" value=" 导出中奖名单 " style="display:none;"  class="btn" onclick="location.href='<?=$this->baseurl?>&m=outputExcel<?=$game_sign?>'" />
    ...
    <?php }else{ ?> 
    <font color="#FF0000">(活动已结束，中奖者的领奖地址为：<?=base_url('index.php?c=receive_prize')?><?=$game_sign?>)</font>
    <?php   } ?>
	...
	<form action="<?=$this->baseurl?>&m=fahuo" method="post">
		...
		<table width="100%" border="0" cellpadding="3" cellspacing="0"
			...
			<tr>
				...
				<th width="30">排序</th>
				<th >头像</th>
                <th class="td_l" align="left">微信昵称</th>
				<th class="td_l"  align="left">积分</th>
                <th align="left">中奖等级</th>
				<th class="td_l"  align="left">姓名</th>
                <th class="td_l"  align="left">手机号</th>				
				<th class="td_l"  align="left">邮寄地址</th>
    ...
    <tr class="sortTr">
				...
				<td  class="td_c" ><?php if(!$r['islock']){?>
						<input type="checkbox" name="delete[]" value="<?=$r['id']?>"class="checkbox" />
					<?php }?>
				</td>
				<td  class="td_c"><?=$key+1?></td>
				<td class="td_c"><img src="<?=$r['head_img']?>" width="50" height="50" /></td>
                <td><?=$r['nickname']?></td>
                <td><?=$r['max_score']?></td>
