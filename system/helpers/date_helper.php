...
if (! function_exists('now')) {
    ...
    function now ()
    {
        ...
        if (strtolower($CI->config->item('time_reference')) == 'gmt') {
            $now = time();
            $system_time = mktime(gmdate("H", $now), gmdate("i", $now), 
...
if (! function_exists('local_to_gmt')) {
    ...
    function local_to_gmt ($time = '')
    {
        ...
        return mktime(gmdate("H", $time), gmdate("i", $time), 
                gmdate("s", $time), gmdate("m", $time), gmdate("d", $time), 
                gmdate("Y", $time));
    }
}
...
if (! function_exists('mysql_to_unix')) {
    ...
    function mysql_to_unix ($time = '')
    {
        ...
        // YYYYMMDDHHMMSS
        return mktime(substr($time, 8, 2), substr($time, 10, 2), 
                substr($time, 12, 2), substr($time, 4, 2), substr($time, 6, 2), 
                substr($time, 0, 4));
    }
}
...
if (! function_exists('timezone_menu')) {
    ...
    function timezone_menu ($default = 'UTC', $class = "", $name = 'timezones')
    {
        ...
        foreach (timezones() as $key => $val) {
            ...
            $menu .= "<option value='{$key}'{$selected}>" . $CI->lang->line(
                    $key) . "</option>\n";
        }
