...
class CI_Jquery extends CI_Javascript
{
    ...
    function _updater ($container = 'this', $controller, $options = '')
    {
        ...
        $controller = (strpos('://', $controller) === FALSE) ? $controller : $this->CI->config->site_url(
                $controller);
        ...
        if ($this->CI->config->item('javascript_ajax_img') == '') {
            ...
            $loading_notifier = '<img src=\'' .
                     $this->CI->config->slash_item('base_url') .
                     $this->CI->config->item('javascript_ajax_img') .
                     '\' alt=\'Loading\' />';
        }
        ...
        if ($options != '') {
            $request_options .= ", {";
            $request_options .= (is_array($options)) ? "'" .
                     implode("', '", $options) . "'" : "'" .
                     str_replace(":", "':'", $options) . "'";
            $request_options .= "}";
        }
    ...
    function _zebraTables ($class = '', $odd = 'odd', $hover = '')
    {
        ...
        if ($hover != '') {
            $hover = $this->hover("table{$class} tbody tr", 
                    "$(this).addClass('hover');", 
    ...
    function corner ($element = '', $corner_style = '')
    {
        ...
        return "$(" . $this->_prep_element($element) . ").corner(" .
                 $corner_style . ");";
    }
    ...
    function sortable ($element, $options = array())
    {
        ...
        return "$(" . $this->_prep_element($element) . ").sortable({" .
                 $sort_options . "\n\t});";
    }
    ...
    {
        $this->jquery_code_for_compile[] = "\t$(" . $this->_prep_element($table) .
                 ").tablesorter($options);\n";
    }
