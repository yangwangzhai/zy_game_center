...
class raceDog extends CI_Controller
{
    ...
    public function index () {	
		...
		$state_base64 = base64_encode('http://h5game.gxtianhai.cn/mntvdb/gamecenter/index.php?d=racedog&c=raceDog&m=getUserInfo&CID='.$this->ChannelID.'&AID='.$this->ActiveID.'&RID='.$this->RoomID);
  ...
  4 => '4',  3 2 5 4 1
	...
	function consumeYD($smokeBeans, $openid, $desc, $gameid){
		...
		if($user_obj['status'] == '0'){				
			...
			if($smokeBeansCount < abs($smokeBeans) || $smokeBeansCount == abs($smokeBeans)){// 如果扣除的烟豆数少于剩余的烟豆数就扣完
				...
				if($openid == 'zyracedog20160418'){
					...
					if($yd['status'] == '0'){				
						$smokeBeansCount = $yd['smokeBeansCount'];
						$this->db->query( "UPDATE zy_racedog_player SET total_gold = ".$smokeBeansCount." WHERE  openid= 'zyracedog20160418' ");
					}
				}
			}
		}
		...
		//获取活动信息
		$Active = $this->my_common_model->get_active_list($this->ActiveID);
		...
		$data['desc'] = $desc.' 活动名称：'.$Active['ActiveName'].'活动ID：'.$Active['ActiveID'];
		$data['app_id'] = $this->zlapp_id;
		$data['openid'] = $openid;
		...
		if($now_time < $start_time || $now_time > $end_time || $now_time == $end_time || $isStop == 1 ){
			...
			if(!empty($openid ) && $openid != '0000'){
				...
				if(!$accept){
                    ...
                    if($Active){
                        $this->db->insert('zy_gamelist_user',array('Openid'=>$openid,'Nickname'=>$nickname,'ChannelID'=>$ChannelID,'ActiveID'=>$ActiveID,'GameID'=>$Active['GameID'],'Num'=>'','UpdateTime'=>time(),'AddTime'=>time()));
                    }
