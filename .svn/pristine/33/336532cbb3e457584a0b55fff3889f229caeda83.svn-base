 ...
 <title>疯狂赛狗</title>   
    ...
    <style>
        body, canvas, div {
          ...
          .body{ background-image:url('static/gameroom/racedog/res/Normal/loading_bg.jpg') ;background-attachment:fixed;background-repeat:no-repeat;background-size:cover;-moz-background-size:cover;-webkit-background-size:cover;}
    </style>
</head>
<body style="padding:0; margin: 0; background: #000;">
<script src="http://192.168.1.178:<?=$SocketPort?>/socket.io/socket.io.js"></script>
<script src="http://static.gxtianhai.cn/racedog/static/js/jquery-1.7.1.min.js"></script>
<script src="static/gameroom/racedog/res/loading.js"></script>
...
var gameInfo = {ChannelID:<?=$ChannelID?>,ActiveID:<?=$ActiveID?>,RoomID:<?=$RoomID?>};
...
var SocketPort = <?=$SocketPort?>;
    ...
    function check_id(){
         ...
         $.ajax({
            type: "POST",
            url: "index.php?d=racedog&c=raceDog&m=check_id&ChannelID=<?=$ChannelID?>&ActiveID=<?=$ActiveID?>&RoomID=<?=$RoomID?>",
     ...
     function postBetData(dataBet) {
        ...
        $.ajax({
            type: "POST",
            url: "index.php?d=racedog&c=raceDog&m=save_beton&ChannelID=<?=$ChannelID?>&ActiveID=<?=$ActiveID?>&RoomID=<?=$RoomID?>",
          //  contentType: "application/json", //必须有
            dataType: "json", //表示返回值类型，不必须
    ...
    function postBetAgain(dataBet) {
        ...
        $.ajax({
            type: "POST",
            url: "index.php?d=racedog&c=raceDog&m=save_beton_again&ChannelID=<?=$ChannelID?>&ActiveID=<?=$ActiveID?>&RoomID=<?=$RoomID?>",
            //  contentType: "application/json", //必须有
            dataType: "json", //表示返回值类型，不必须
    ...
    function getSpeed(){
     ...
     function postGift(data){
         $.ajax({
             type: "POST",
             url: "index.php?d=racedog&c=raceDog&m=save_gift&ChannelID=<?=$ChannelID?>&ActiveID=<?=$ActiveID?>&RoomID=<?=$RoomID?>",
             //  contentType: "application/json", //必须有
             dataType: "json", //表示返回值类型，不必须
  ...
  wx.ready(function () {
    ...
    wx.onMenuShareAppMessage({
        title: '疯狂赛狗', // 分享标题
        desc: '疯狂赛狗', // 分享描述
        link: 'http://h5game.gxtianhai.cn/gamecenter/index.php?d=racedog&c=raceDog&ChannelID=<?=$ChannelID?>&ActiveID=<?=$ActiveID?>&RoomID=<?=$RoomID?>', // 分享链接
    ...
    wx.onMenuShareTimeline({
        title: '疯狂赛狗', // 分享标题
        link: 'http://h5game.gxtianhai.cn/gamecenter/index.php?d=racedog&c=raceDog&ChannelID=<?=$ChannelID?>&ActiveID=<?=$ActiveID?>&RoomID=<?=$RoomID?>',
        imgUrl: '<?=base_url('static/gameroom/racedog/images/loadingBG.png');?>', // 分享图标
        success: function () { 
            // 用户确认分享后执行的回调函数
        },
...
<script src="static/gameroom/raceDog/CCBoot.js"></script>
<script cocos src="static/gameroom/racedog/main.js"></script>
