...
class notice extends Content
{
    ...
    public function index(){
        ...
        $query = $this->db->query(
            "SELECT COUNT(*) AS num FROM zy_user WHERE status=0 and $searchsql ");
	...
	/*$rank_sql = 'SELECT M.* ,C.openID,C.id,C.head_img,C.status,C.nickname,C.tel  FROM ( SELECT A.*,@rank:=@rank+1 AS pm FROM ' ;
		...
		$rank_sql = "select * FROM zy_user where status = 0 and $searchsql ORDER BY score desc limit  $offset,$per_page";
		$query = $this->db->query( $rank_sql );
		$result = $query->result_array();	
