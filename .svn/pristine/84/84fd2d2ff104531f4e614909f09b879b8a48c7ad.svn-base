...
class CI_Javascript
{
    ...
    public function __construct ($params = array())
    {
        ...
        $this->CI->load->library('javascript/' . $js_library_driver, 
                array(
                        'autoload' => $autoload
                ));
        ...
        log_message('debug', 
                "Javascript Class Initialized and loaded.  Driver used: $js_library_driver");
    }
    ...
    function external ($external_file = '', $relative = FALSE)
    {
        if ($external_file !== '') {
            ...
            if ($this->CI->config->item('javascript_location') != '') {
                $this->_javascript_location = $this->CI->config->item(
                        'javascript_location');
            }
        }
        
        if ($relative === TRUE or strncmp($external_file, 'http://', 7) == 0 or
                 strncmp($external_file, 'https://', 8) == 0) {
    ...
    {
        $str = '<script type="text/javascript" charset="' .
