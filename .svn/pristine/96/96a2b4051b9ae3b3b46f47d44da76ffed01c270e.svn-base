...
class CI_Xmlrpcs extends CI_Xmlrpc
{
    ...
    function parseRequest ($data = '')
    {
        ...
        if (! xml_parse($parser, $data, 1)) {
            ...
            return new XML_RPC_Response(0, $this->xmlrpcerr['invalid_return'], 
            ...
            for ($i = 0; $i < count($parser_object->xh[$parser]['params']); $i ++) {
                if ($this->debug === TRUE) {
                    $plist .= "$i - " .
                             print_r(
                                    get_object_vars(
                                            $parser_object->xh[$parser]['params'][$i]), 
                                    TRUE) . ";\n";
                }
    ...
    function _execute ($m)
    {
        ...
        if (! isset($this->methods[$methName]['function'])) {
            return new XML_RPC_Response(0, $this->xmlrpcerr['unknown_method'], 
        ...
        if ($system_call === TRUE) {
            ...
            if ($objectCall &&
                ...
                return new XML_RPC_Response(0, 
                     ...
                     ! is_callable($this->methods[$methName]['function'])) {
                return new XML_RPC_Response(0, 
                        $this->xmlrpcerr['unknown_method'], 
                        $this->xmlrpcstr['unknown_method']);
            }
    ...
    function methodSignature ($m)
    {
        ...
        if (isset($this->methods[$method_name])) {
            ...
            $r = new XML_RPC_Response(0, $this->xmlrpcerr['introspect_unknown'], 
                    $this->xmlrpcstr['introspect_unknown']);
        }
        return $r;
    }
    ...
    function methodHelp ($m)
    {
        ...
        if (isset($this->methods[$method_name])) {
            ...
            return new XML_RPC_Response(0, 
                    $this->xmlrpcerr['introspect_unknown'], 
                    $this->xmlrpcstr['introspect_unknown']);
        }
    }
